...
- The Clawpals desktop app must live in the **system tray / notification area** (the expandable tray), not the taskbar.
...
### Style lock (mandatory for all Clawpals)
- See `docs/clawpals-style-guide.md` (v1).
...
4. **Remote install + projection (`clawpals` OpenClaw skill)**
   ...
   - One-command UX: user provides target IP/hostname; OpenClaw downloads the Clawpals binary, installs it on the target, pairs runtime, and projects the avatar.
